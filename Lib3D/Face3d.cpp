...
void CFace3d::CalculateNormal()
{
	...
	u.Inner(v);
	u.Set(nFlip*u.x(), nFlip*u.y(), nFlip*u.z());
	m_Normal.Set(u);
...
void CFace3d::reorient() {
	CVertex3d* pVertex1 = m_pVertex[1];
	CVertex3d* pVertex2 = m_pVertex[2];
  m_pVertex[1] = pVertex2;
	m_pVertex[2] = pVertex1;
}
...
float CFace3d::aspectRatio() {
	// the length of three edges
	float a=Distance(m_pVertex[0],m_pVertex[1]),
		    b=Distance(m_pVertex[0],m_pVertex[2]),
				c=Distance(m_pVertex[1],m_pVertex[2]);
	// outer radius
	float outR = 0.25f*a*b*c/Area();
	// perimeter length
  float pr = Perimeter();
	// inner radius
	float InR = Area()*2/pr;
	return 0.5f*outR/InR;
}
