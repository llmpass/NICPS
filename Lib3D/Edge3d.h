...
public :
	...
	//for each edge, we must store its corresponding value in opt matrix
	//that is coefficients for xixj, yiyj, zizj
	float xixj, yiyj, zizj;
	//index of its start and end vertices
	int start, end;
	...
	CEdge3d::CEdge3d(CVertex3d *pVertex1, CVertex3d *pVertex2, int s, int e);
	...
	float len;
	void ComputeLength() 
	{len = (float)sqrt((m_pVertex1->x()-m_pVertex2->x())*(m_pVertex1->x()-m_pVertex2->x()) + 
				       (m_pVertex1->y()-m_pVertex2->y())*(m_pVertex1->y()-m_pVertex2->y()) );};
