							...
							double *pSum)
{
	...
	for(int i=0;i<size-1;i++)
	{
		...
		{
			CFace3d *pFaceTest = array[j];
			if(pFace->HasNeighbor(pFaceTest)) sum += SinAngle(pFace,pFaceTest);
		}
	}
...
*         zero, otherwise.
*/
double leftOfLineFast(double xa, double ya, double xb, double yb, double xc, double yc) {
	double acx = xa - xc;
	double bcx = xb - xc;
	double acy = ya - yc;
	double bcy = yb - yc;
...
*/
void centerCircle(float xa, float ya,
									float xb, float yb,
									float xc, float yc,
									float* po) {
	double acx = xa - xc;
	...
	double scale = 0.5/leftOfLineFast(xa,ya,xb,yb,xc,yc);
...
// GetMeanArea
//**************************************
double GetSumArea(CArray3d<CFace3d> *pArrayFace)
{
	int size = pArrayFace->GetSize();
	ASSERT(size > 0);
	double sum = 0.0f;
