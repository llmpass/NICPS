										 ...
										 CSceneGraph3d *pSceneGraph)
{
	...
	while(OffsetToStringBeginLine("DEF")) ReadMesh(pSceneGraph);
...
int CParserVrml::ReadMesh(CSceneGraph3d *pSceneGraph)
{
  ...
  // diffuseColor 0.5686 0.1098 0.6941
	...
	if(OffsetToStringBefore("texture ImageTexture","geometry"))
	{
        ...
        //pTexture->ReadFileBMP("F:\Luming\bone/ alignment\easyViewer\checkerboard.bmp");
													...
													int *pNbTextureCoordinate /* = NULL */)
{
	...
	do
	{
		...
		int temp;
		ReadWord();
		temp = sscanf(m_pBufferWord,"%d,",&v1);
		if (v1 == -1) {
			ReadWord();
      temp = sscanf(m_pBufferWord,"%d,",&v1);
		}
		success = temp;
		ReadWord();
		temp = sscanf(m_pBufferWord,"%d,",&v2);
		if (v2 == -1) {
			ReadWord();
      temp = sscanf(m_pBufferWord,"%d,",&v2);
		}
		success &= temp;
		ReadWord();
		temp = sscanf(m_pBufferWord,"%d,",&v3);
		if (v2 == -1) {
			ReadWord();
      temp = sscanf(m_pBufferWord,"%d,",&v3);
		}
